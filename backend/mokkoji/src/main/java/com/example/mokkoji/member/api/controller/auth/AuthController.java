package com.example.mokkoji.member.api.controller.auth;

import com.example.mokkoji.member.api.entity.auth.AuthReqModel;
...
public class AuthController {
    ...
    private final AppProperties appProperties;  // 애플리케이션 설정 정보
    ...
    public ApiResponse login(HttpServletRequest request, HttpServletResponse response, @RequestBody AuthReqModel authReqModel) {
        ...
        CookieUtil.deleteCookie(request, response, REFRESH_TOKEN);  // (request, response, name)
        CookieUtil.addCookie(response, REFRESH_TOKEN, refreshToken.getToken(), cookieMaxAge);  // (response, name, value, maxAge)
